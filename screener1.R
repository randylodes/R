...
# Experimenting with specifying a 'to' date
# Probably will want a way to discover last trading day of each month
# ...or 2nd to last
...
# A function for getting Adjusted Close values (instead of Cl for Close)
Adj <- function(x) 
{
...
getSymbols("SPY", from="2014-01-01", to=to_date)
...
for(symbol_name in symbols)
{
   ...
   if (!exists(symbol_name))
      ...
      getSymbols(symbol_name, from="2014-01-01", to=to_date)
	  }

   # Create an XTS object of daily data for the current symbol  
   data1 <- get(symbol_name)
   
   # Testing this line for adjusted data
   # data1 <- adjustOHLC(data1)
   
   # Calculate the 3 values for this symbol
   ...
   # Same as above block of code, but using 'adjusted close' data from Yahoo
   RetA <- as.numeric(tail(round(ROC(Adj(data1), n=timeA, type="discrete"), 3)*100, 1))
   RetB <- as.numeric(tail(round(ROC(Adj(data1), n=timeB, type="discrete"), 3)*100, 1))
   Vol <- round(sd(tail(ROC(Adj(data1), type="discrete"), timeV))*sqrt(252), 3)*100
   Cor <- as.numeric(round(cor(Adj(data1), Adj(SPY)), 3)*100)
